  ...
  const lignes = useMemo(() => {
    return ruptures
      ...
      .map((art) => {
        ...
        return { art, stock, objectif, aCommander: objectif > 0 ? Math.max(0, objectif - stock) : null };
      })
        ...
        <tbody>
          {lignes.map((l) => (
            <tr key={l.art.code}>
              ...
              <td>{l.art.fournisseur}</td>
              <td className="num">{l.stock}</td>
              <td className="num">{l.objectif > 0 ? l.objectif : "—"}</td>
              <td className="num strong">{l.aCommander !== null ? l.aCommander : "—"}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
function KardexView({ mouvements, stockByCode, selectedCode, setSelectedCode, addMouvement }) {
  ...
  function submit(e) {
    ...
    const payload = { article: art.code, type, quantite: q, date, motif, reference: reference.trim() };
    const dispo = stockByCode[art.code] || 0;
    if (type === "sortie" && q > dispo) {
      setPending(payload);
      return;
    }
    doSubmit(payload);
          ...
          <form className="kx-form" onSubmit={submit}>
            ...
            {pending && (
              <div className="kx-warning">
                <p>
                  La quantité saisie ({pending.quantite}) est supérieure au stock disponible ({stockByCode[art.code] || 0} unité(s)).
...
function CatalogueView({ stockByCode, objectifsByCode, setObjectif, onOpenArticle }) {
  ...
  const filtered = useMemo(() => {
    ...
    return ARTICLES.filter((a) => {
      ...
      if (qvFiltre === "definie" && objectif <= 0) return false;
      if (qvFiltre === "nondefinie" && objectif > 0) return false;
      return true;
    });
        ...
        <div className="kx-filters-advanced">
          ...
          <label>
            ...
            <select value={qvFiltre} onChange={(e) => setQvFiltre(e.target.value)}>
              ...
              <option value="definie">Définie</option>
              <option value="nondefinie">Non définie</option>
            </select>
          </label>
